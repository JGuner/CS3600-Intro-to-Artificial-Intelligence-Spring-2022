...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        observation = busters.getObservationProbability(noisyDistance, manhattanDistance(pacmanPosition,ghostPosition))
        return observation
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for initghostPos in self.allPositions:
            finalposDistribution = self.getPositionDistribution(gameState, initghostPos)
            finalPos[initghostPos] = finalposDistribution

        for finalghostPos in self.allPositions:
            prob = 0
            for initghostPos in self.allPositions:
                prob = prob + self.beliefs[initghostPos] * finalPos[initghostPos][finalghostPos]
            temp_beliefs[finalghostPos] = prob
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        posNum = len(self.legalPositions)
        partsperPos = self.numParticles // posNum

        for pos in self.legalPositions:
            for p in range(partsperPos):
                self.particles.append(pos)

        if self.numParticles % posNum != 0:
            for p in range(self.numParticles % posNum):
                self.particles.append(self.legalPositions[p])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            observeProb = self.getObservationProb(observation, gameState.getPacmanPosition(), particle,
                                                 self.getJailPosition())
            if particle in disDist:
                disWeight = disDist[particle]
                disWeight = disWeight + observeProb
                disDist[particle] = disWeight
            else:
                disDist[particle] = observeProb
        disDist.normalize()
        if disDist.total() > 0:
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        if self.numParticles % posNum != 0:
            for i in range(self.numParticles % posNum):
