...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for distr in livingGhostPositionDistributions:
            position = distr.argMax()
            distance = self.distancer.getDistance(pacmanPosition, position)
            if distance < closestD:
                closestP = position
                closestD = distance
        bestAction = None
        closestDAfterAct = 88888

        for action in legal:
            posAfter = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(closestP, posAfter)
            if distance < closestDAfterAct:
                closestDAfterAct = distance
                bestAction = action
