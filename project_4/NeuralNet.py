...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for index, weight in enumerate(self.weights):
            temp = weight + (alpha * inActs[index] * delta)
            weights.append(temp)
            totalModification += abs(weight - temp)
            weight = temp
        inActs.pop(0)
        self.weights = weights
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for i in self.layers:
            inputVector = output[ind]
            output.append([])
            for k in i:
                output[ind + 1].append(k.sigmoidActivation(inputVector))
            ind +=1
        return output
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = gPrime * sum([nextLayer[i].weights[neuronNum + 1] * deltas[0][i] for i in range(len(nextLayer))])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while boolean:
        ...
        if iteration % 10 == 0:
            print
            'Iteration %d; Training Error %f; Weight change %f' % (iteration, trainError, weightMod)
        else:
            print
            'Finish',

        boolean = iteration < maxItr and weightMod > weightChangeThreshold
    ...
    for i in examplesTest:
        output = nnet.feedForward(i[0])
        norm = [round(val) for val in output[-1]]
        if norm == i[1]:
            testCorrect += 1.0
        else:
            testError += 1.0
